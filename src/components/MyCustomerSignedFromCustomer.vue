			...
			<div class="base-info">
				...
				<div class="form-warpper">
					<el-form :model="signed" ref="base" label-width="100px" class="demo-ruleForm">
						...
						<!--<el-form-item label="路线" v-if="signed.productnum==1">
              <el-select v-model="signed.address" placeholder="请选择路线">
                <el-option v-for="item in addressList"  :label="item.name" :value="item.id"></el-option>
              </el-select>
            </el-form-item>-->

						<el-form-item label="平台区域" prop="cityCode" v-if="signed.productnum==1 || signed.productnum==2">
							<el-cascader placeholder="请选择：试试搜索？" expand-trigger="hover" :options="cityOptions" filterable v-model="signed.cityCode" @change="handleCityCodeChange" :props="{value:'code',label:'name',children:'list'}">
							</el-cascader>
						</el-form-item>
						...
						<el-form-item label="是否拿票" v-if="signed.productnum==1">
							<el-tooltip placement="top">
								<el-switch v-model="signed.getTicket" on-color="#13ce66" off-color="#ff4949" on-value="true" on-text="是" off-text="否" off-value="false">
								</el-switch>
							</el-tooltip>
						</el-form-item>

						<el-form-item label="旅游票区间" v-if="signed.productnum==1">
							<el-input type="number" v-model="signed.priceBegin"></el-input><br /> 至
							<br />
...
//					id: this.goid,
			...
			getProduct() {
				this.$http.get('/v2/crm/config/list/type').then(res => {
					...
					//        console.log('二级菜单',this.product2);
				}).catch(res => {})
			},
			getAddressList() {
			...
			getUpdateInfo() {
				this.$http.get('/v1/aut/crm/customer/id', {
					...
					console.log('获取客户详情', res.body.data);
					var k = res.body.data;
...
//					this.signed.storyAccount = k.bookAccount;
		...
		created() {
			this.goid = this.$route.params.customerid;
			this.getUpdateInfo();
